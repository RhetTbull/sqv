...
class TestDatabaseConnection:
    ...
    def test_get_tables(self, db: DatabaseConnection) -> None:
    ...
    def test_get_columns(self, db: DatabaseConnection) -> None:
    ...
    def test_get_row_count(self, db: DatabaseConnection) -> None:
        """Test getting row count."""
        assert db.get_row_count("users") == 5
        assert db.get_row_count("posts") == 4
        assert db.get_row_count("empty_table") == 0

    def test_fetch_rows_basic(self, db: DatabaseConnection) -> None:
    ...
    def test_fetch_rows_with_limit(self, db: DatabaseConnection) -> None:
        """Test row fetching with limit."""
        columns, rows = db.fetch_rows("users", limit=2)
        assert len(rows) == 2

    def test_fetch_rows_with_offset(self, db: DatabaseConnection) -> None:
    ...
    def test_fetch_rows_with_order(self, db: DatabaseConnection) -> None:
    ...
    def test_fetch_rows_with_where(self, db: DatabaseConnection) -> None:
        """Test row fetching with WHERE clause."""
        _, rows = db.fetch_rows("users", where="age > 30")
        assert len(rows) == 2  # Charlie (35) and Eve (32)

    def test_execute_sql_select(self, db: DatabaseConnection) -> None:
    ...
    def test_execute_sql_insert(self, db: DatabaseConnection) -> None:
    ...
    def test_execute_sql_error(self, db: DatabaseConnection) -> None:
        """Test executing invalid SQL raises error."""
        with pytest.raises(sqlite3.Error):
            db.execute_sql("SELECT * FROM nonexistent_table")

    def test_execute_sql_syntax_error(self, db: DatabaseConnection) -> None:
...
class TestPagination:
    ...
    def test_pagination(self, large_db_path: Path) -> None:
